...
- [X] T037 [P] [US3] Add failing truth-table tests for pure Fitted/Flexible/Improvised resolution, base behavior retention, gained Fitted behavior, lost Fitted disclosure, authored Improvised consequence or explicit `none`, item-definition immutability, and legality across every catalog item and slot type in `tests/unit/garage.test.ts` — `resolveInstallation` was already correctly implemented (Phase 4 groundwork), so this matrix landed GREEN immediately rather than RED
- [X] T038 [P] [US3] Extend failing lap tests for typed base/Fitted/Improvised behavior operations, Flexible base-only behavior, no installation behavior in storage, same-type slot permutation neutrality, and stable existing cooldown/buff ordering in `tests/unit/laps.test.ts` — confirmed RED (6/8 new cases failing) against the old installation-blind `laps.ts`
- [X] T039 [US3] Add failing presentation tests for candidate/current-occupant installation states, exact behavior gained/retained/lost, displacement result, category/origin/tags/cooldown/price/affordability/storage behavior, and explicit no-additional-consequence text in `tests/unit/garagePresentation.test.ts` (depends on T037-T038) — confirmed RED (`garageItemInspector` did not exist yet)

### Implementation for User Story 3

- [X] T040 [US3] Implement pure `resolveInstallation` and enrich placement previews with display-ready behavior/state/loss/displacement facts while treating every category-slot pairing as legal in `src/simulation/garage.ts` (depends on T037-T039) — already implemented in `src/simulation/slots.ts` (garage.ts's preview already consumes it); left in place rather than relocated
- [X] T041 [US3] Apply structured base plus resolved authored installation behavior during deterministic lap simulation and record slot ID, installation state, behavior source, and numeric contribution in `src/simulation/laps.ts` (depends on T038, T040) — added `effectiveItem`/`installationBehaviorSource`; every vehicle-slot item is folded through `resolveInstallation` before its contribution and any buff-boost amplification are computed; storage items are unaffected
- [X] T042 [US3] Extend the persistent inspector and occupant comparison presentation with all required authored, installation, price, affordability, and storage facts in `src/scenes/garagePresentation.ts` and `src/scenes/itemVisualDescriptor.ts` (depends on T039-T041) — added `garageItemInspector`, reusing `itemVisualDescriptor` for cooldown facts
- [X] T043 [US3] Render text/icon/structural Fitted, Flexible, and Improvised badges plus exact preview/result changes for every enabled destination in `src/scenes/PrepareScene.ts` (depends on T042) — per-slot installation-state badge under every held item, plus inspector-driven tooltips (offers disclose both possible outcomes; placed items show the resolved state, gained/lost behavior, and no-consequence text)
- [X] T044 [US3] Run `tests/unit/garage.test.ts`, `tests/unit/laps.test.ts`, and `tests/unit/garagePresentation.test.ts`; confirm the complete item-by-slot matrix, simulation behavior, and inspector comparison cases are GREEN (depends on T040-T043) — 436/436 full-suite tests, tsc, eslint, and build all clean; verified live in-browser (offer tooltip, Fitted badge/tooltip after purchase, unaffordable flag)
