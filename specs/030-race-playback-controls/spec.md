...
1. **Given** a newly opened scored race, **When** playback begins, **Then** it runs at 2× and matches the pre-feature presentation duration.
2. **Given** a newly opened Test Day race, **When** playback begins, **Then** it uses the same 2× default and timing meaning as a scored race.
3. **Given** a race playing at 1×, **When** no input is provided, **Then** every recorded event is processed exactly once in its original order and the race advances to Results only after all cars finish; if one rendered update crosses multiple text-message boundaries, only the final processed message is guaranteed to be human-visible.
...
2. **Given** playback at 2×, **When** the player selects 1×, **Then** presentation switches to the slower readable pace without restarting or jumping backward.
...
- A large or delayed render frame may cross multiple presentation boundaries; all required immutable events are processed exactly once, while only the final text message from that update is guaranteed to remain human-visible under the no-queue rule.
- Recorded events at schedule time zero are published once as an initialization batch before the first positive-time interval is advanced.
- Speed changes during the final moments cannot prevent Results navigation or trigger it twice.
- Leaving and opening another race resets playback to 2×; speed is not remembered between races in this feature.
...
- **FR-001**: Every newly opened scored race and Test Day race MUST begin at the default `2×` playback speed.
...
- **FR-008**: Speed selection MUST be race-local and MUST reset to `2×` whenever a new watched race or Test Day playback begins.
- **FR-009**: Playback MUST publish schedule-time-zero events exactly once during initialization, then publish each lap, checkpoint, item activation, projection, or finish event exactly once when its boundary lies in `(previousScheduleTime, nextScheduleTime]`; when multiple text messages are processed in one rendered update, only the final message is required to remain visible.
...
- **SC-004**: Automated boundary tests observe zero duplicated or omitted time-zero, lap, projection, item, or finish events under rapid and final-moment speed changes, counting events processed in deterministic order rather than requiring every same-update intermediate text message to remain human-visible.
...
- Every race begins at the faster default; players may select the slower readable `1×` option at any time, and no preference is persisted.
