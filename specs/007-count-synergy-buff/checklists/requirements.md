...
- `/speckit.clarify` session (2026-07-27) caught a real logic bug in SC-003 as originally drafted: it claimed comparing an inert-item-present build against an item-absent build would produce "different outcomes," but a count-synergy buff's boost only ever applies to *active* matching items — with no other active receiver present in either build, both would have produced the identical `playerTime`, making the claim false. Fixed by requiring both compared builds to also hold a shared active matching-tag item to receive the boost.
