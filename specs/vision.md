...
- **Post-demo direction** (`specs/skribidi-gap-decisions.md`): the 1v1 ghost model is superseded by N-ghost racing (§1), which raises a real-async-ghost-recording requirement this vision doc already anticipated — race results must be canonical across every viewer, not just deterministic per player (§2, flagged for its own architecture spec). The former "Rival Intel" idea (§8) grew into a full pre-race screen — track preview plus a small number of simple, item-driven pre-race controls for builds that have configurable items (e.g. an adaptive-brakes item exposing a brake-bias slider) — a second, lighter-weight candidate for "a dimension outside the car" alongside season length (§7).
