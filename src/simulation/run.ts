...
import { emptyInstanceBuild, initializeLiveBuild } from "./liveItemInstances";
import { projectInstanceBuild } from "./liveItemInstances";
import { allItemDefinitions } from "./itemPools";
import { createCadenceState, type CadenceState } from "./encounterCadence";
import type { EncounterHistoryInput } from "./historyProjection";
import { settleScrutineering, type ScrutineeringTemplate, type SlotReservation } from "./scrutineering";
import {
...
import {
  ...
  type VehicleBuild,
  type WorldTourState,
  type EncounterType as VarietyEncounterType,
} from "./types";
...
export type EncounterType = VarietyEncounterType | "pvp";
...
export interface EncounterChoice {
  ...
  summary: string;
  /** Feature 034 retained authored presentation identity. */
  variantId?: string;
  title?: string;
}
...
export interface PendingEncounterPayload {
  ...
  variantId?: string;
  /** Retained feature-specific offer/trial payload. Never regenerated by scenes. */
  data?: unknown;
}
...
export interface RunHistoryEntry {
  ...
  varietyOutcome?: EncounterHistoryInput;
}

export const ENCOUNTER_VARIETY_RUN_VERSION = "encounter-variety-v1" as const;

export interface PendingScrutineeringState {
  pendingEffectId: string;
  sourceEncounterId: string;
  template: ScrutineeringTemplate;
  reservation: SlotReservation;
  recoveredInstance: ItemInstance;
  targetScoredStage: number;
  status: "pending" | "race-committed" | "returned" | "settled";
}
...
export interface Run {
  ...
  /** Feature 034 versioned instance/cadence authority. Missing means typed legacy recovery. */
  encounterVarietyVersion?: typeof ENCOUNTER_VARIETY_RUN_VERSION;
  instanceBuild?: InstanceBuild;
  itemInstanceOrdinal?: number;
  cadenceState?: CadenceState;
  encounterRevision?: number;
  encounterHistory?: EncounterHistoryInput[];
  pendingScrutineering?: PendingScrutineeringState | null;
}

export type EncounterVarietyCompatibility =
  | { kind: "compatible" }
  | { kind: "legacy-unavailable"; reason: "missing-version" | "missing-instance-build" | "unknown-version" };

/** Unknown/legacy state is never silently migrated or guessed. */
export function validateEncounterVarietyCompatibility(run: Run): EncounterVarietyCompatibility {
  if (!run.encounterVarietyVersion) return { kind: "legacy-unavailable", reason: "missing-version" };
  if (run.encounterVarietyVersion !== ENCOUNTER_VARIETY_RUN_VERSION) {
    return { kind: "legacy-unavailable", reason: "unknown-version" };
  }
  if (!run.instanceBuild) return { kind: "legacy-unavailable", reason: "missing-instance-build" };
...
export function createRun(input: CreateRunInput): Run {
  const initialized = initializeLiveBuild(input.build, input.runId);
  if (initialized.kind !== "ok") throw new RunTransitionError("invalid-action", initialized.reason);
  const run: Run = {
    id: input.runId,
    seed: input.seed,
    ...
    encounterVarietyVersion: ENCOUNTER_VARIETY_RUN_VERSION,
    instanceBuild: initialized.instanceBuild,
    itemInstanceOrdinal: initialized.nextOrdinal,
    cadenceState: createCadenceState(),
    encounterRevision: 0,
    encounterHistory: [],
    pendingScrutineering: null,
  };
...
export function createUnavailableRun(input: Omit<CreateRunInput, "rng">): Run {
  return {
    ...
    reputation: 0,
    encounterVarietyVersion: ENCOUNTER_VARIETY_RUN_VERSION,
    instanceBuild: emptyInstanceBuild(input.build),
    itemInstanceOrdinal: 1,
    cadenceState: createCadenceState(),
    encounterRevision: 0,
    encounterHistory: [],
    pendingScrutineering: null,
...
export function completeNonPvpEncounter(
  ...
  const historyEntry: RunHistoryEntry = {
    ...
    varietyOutcome: [...(run.encounterHistory ?? [])].reverse().find((entry) => entry.encounterId === active.id),
  };
  return advanceRun(
...
export function completePvpEncounter(
  ...
  // Scrutineering is consumed only by a scored race. Test Day and Exhibition
  // never call this settlement path, so they cannot consume the reservation.
  if (next.pendingScrutineering && next.instanceBuild) {
    const pending = next.pendingScrutineering;
    const settled = settleScrutineering(next.instanceBuild, pending.reservation, pending.recoveredInstance);
    if (settled.kind === "settled") {
      const projected = projectInstanceBuild(
        settled.build,
        new Map(allItemDefinitions().map((item) => [item.id, item])),
      );
      if (projected) {
        next = {
          ...next,
          instanceBuild: settled.build,
          build: projected,
          pendingScrutineering: null,
          encounterRevision: (next.encounterRevision ?? 0) + 1,
          encounterHistory: (next.encounterHistory ?? []).map((entry) =>
            entry.encounterId === pending.sourceEncounterId
              ? { ...entry, outcome: "accepted" as const, pendingCategory: null }
              : entry,
          ),
        };
      }
